...
| SPR-020 | CKO Local Knowledge Finder Desktop Interface | [Desktop interface plan](SPR-020_CKO_LOCAL_KNOWLEDGE_FINDER_DESKTOP_INTERFACE_PLAN.md) | [REV-008 pilot recovery review](../reviews/REV-008_CKO_LOCAL_KNOWLEDGE_FINDER_PILOT_RECOVERY_REVIEW.md); [INC-GUI-001A implementation report](SPR-020_INC-GUI-001A_IMPLEMENTATION_REPORT.md); INC-GUI-001B record in the plan | `INC-GUI-001B COMPLETED / FIRST FUNCTIONAL DESKTOP INTERFACE / PUBLIC DISTRIBUTION BLOCKED` |
...
`SPR-020` has its shared application foundation (`INC-GUI-001A`) and first functional
desktop interface (`INC-GUI-001B`) implemented. Public distribution and every later
