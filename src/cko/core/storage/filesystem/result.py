...
class FilesystemResult:
    ...
    def __post_init__(self) -> None:
        ...
        if self.connector_result.connector_id != self.storage_result.storage_id:
            raise StorageException("filesystem result identifiers differ")
        if self.connector_result.success != self.storage_result.success:
            raise StorageException("filesystem result success values differ")
        if self.schema_version != FILESYSTEM_SCHEMA_VERSION:
            raise StorageException("unsupported FilesystemResult version")
    ...
    def from_dict(cls, payload: Mapping[str, object]) -> Self:
        """Deserialize a strict filesystem result envelope."""
        expected = {
            "schema_version",
            "model",
            "connector_result",
        ...
        if not isinstance(payload, Mapping) or set(payload) != expected:
            raise StorageException("invalid filesystem_result envelope")
        if (
            payload.get("schema_version") != FILESYSTEM_SCHEMA_VERSION
            or payload.get("model") != "filesystem_result"
        ):
            raise StorageException("invalid filesystem_result envelope")
        ...
        if not isinstance(connector, Mapping) or not isinstance(storage, Mapping):
            raise StorageException("invalid filesystem result models")
        return cls(
    ...
    def from_json(cls, payload: str) -> Self:
        """Deserialize a filesystem result from strict JSON."""
        try:
            decoded = json.loads(payload)
        except (TypeError, json.JSONDecodeError) as error:
            raise StorageException("filesystem result JSON is invalid") from error
        if not isinstance(decoded, Mapping):
            raise StorageException("filesystem result JSON is invalid")
        return cls.from_dict(decoded)


__all__ = ["FilesystemResult"]
